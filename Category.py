...
class categoryClass:
    def __init__(self, root):
        ...
        self.im1 = self.im1.resize((1500, 500), Image.BILINEAR)


        self.im1=ImageTk.PhotoImage(self.im1)

        self.lbl_im1=Label(self.root,image=self.im1)
        self.lbl_im1.place(x=10,y=250)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="" :
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Categgory already present,try different",parent=self.root)
                else:
